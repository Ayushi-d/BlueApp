...
extension String {
    ...
    struct Regex {
        ...
        FacebookRegex : String = "^(https?://)?(www\\.)?facebook.com([-a-z0-9]{1,63}\\.)*?[a-z0-9][-a-z0-9]{0,61}[a-z0-9]\\.[a-z]{2,6}(/[-\\w@\\+\\.~#\\?&/=%]*)?$",
    ...
    func isValidUrl(urlRegEx: String = Regex.WebUrl) -> Bool {
    ...
    /// To check Alpha string
    ///
    /// ## HOW TO USE:
    ///     "Sample".isValidAlpha() // returns false
    ///
    /// - Parameter text: String
    /// - Returns: true for valid and false
    func isValidAlpha() -> Bool {
        let urlRegEx = Regex.AlphanumericRegex

...
class PickerView: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func addPicker(_ controller: UIViewController, onTextField: UITextField, pickerArray: [String], withCompletionBlock: @escaping pickerCompletionBlock) {
        ...
        keyboardDateToolbar.barTintColor = UIColor.Color.buttonColor
        ...
        done.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.sourceSansProRegular(size: 16)], for: UIControl.State.normal)
        done.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.sourceSansProRegular(size: 16)], for: UIControl.State.highlighted)
        done.tintColor = UIColor.white

        let cancel = UIBarButtonItem(title: "Cancel", style: .done, target: self, action: #selector(pickerCancel))
        cancel.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.sourceSansProRegular(size: 16)], for: UIControl.State.normal)
        cancel.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.sourceSansProRegular(size: 16)], for: UIControl.State.highlighted)

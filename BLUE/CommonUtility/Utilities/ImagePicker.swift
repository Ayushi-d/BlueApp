...
class ImagePicker: NSObject, UINavigationControllerDelegate {
    ...
    public func showImagePicker(_ controller: UIViewController, sender: UIView? = nil, type: [String], isEditing: Bool = true, completion: @escaping ImagePickerCompletionBlock) {
        // Assign the Block
        didCompletePicking = completion

        // Set Message
        let strMessage = "Select Option"
        ...
        alert.view.tintColor = UIColor.Color.buttonColor
        // Add Cancel Action
...
extension ImagePicker {
    ...
    fileprivate func showAlertForEnableAccessPermission(_ controller: UIViewController, message: String) {
        Utility.delay(0) {
            UIAlertController.showAlert(controller: controller, message: message, cancelButton: "Cancel", otherButtons: ["Settings"]) { index, _ in
                if index == 0 {
    ...
    public func usePhotoLibrary(_ controller: UIViewController, type: [String], isEditing: Bool) {
        handlePhotoLibraryStatus(PHPhotoLibrary.authorizationStatus(), controller: controller, type: type, isEditing: isEditing)
    }
...
//            AlertMesage.show(.error, message: "Camera is not supported in this device.")
            UIAlertController.showAlertWithOkButton(controller: controller, message: "Camera is not supported in this device.", completion: nil)
    ...
    fileprivate func openNativeCamera(_ controller: UIViewController, type: [String], isEditing: Bool) {
        DispatchQueue.main.async {
            ...
            self.imagePicker.delegate = self
            if !UIImagePickerController.isSourceTypeAvailable(.camera) {
                controller.showCamaeraNotAvailable()
                return
            }
            controller.present(self.imagePicker, animated: true, completion: nil)
        }

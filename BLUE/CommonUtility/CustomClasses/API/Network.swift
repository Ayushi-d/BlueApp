...
class Network {
    ...
    class func request(_ target: Service, isShowLoader: Bool = true, showError: Bool = true, completion: @escaping Completion) -> Cancellable? {
        ...
        if !Connectivity.shared.isConnected {
            AlertMesage.showInternetNotConnected(message: Localizable.info.noInternetConnetedTitle)
            completion(nil)
            return nil
        }
            ...
            case .success(let response):
                ...
                case 200...300:
                    ...
                    if let aDictData = aResponse[API.Response.data]?.dictionary, !aDictData.isEmpty {
                        ...
                        completion(json)
                    } else if let aArrData = aResponse[API.Response.data]?.array, !aArrData.isEmpty {
                        ...
                        // Array Data is received from the server.
                        completion(json)
                    }else if let aStringData = aResponse[API.Response.message]?.string, !aStringData.isEmpty {
                        // Array Data is received from the server.
                        completion(json)
                    }
                    else {

...
class FBManager: NSObject {
        ...
        let sendRequestPostData = {() -> Void in
            if AccessToken.current?.hasGranted(permission: "publish_actions") ?? false {
                if txtMessage.count>0 && !img!.isKind(of: UIImage.self) {
                    ...
                    GraphRequest(graphPath: "me/feed", parameters: ["message": txtMessage], httpMethod: .post).start(completionHandler: { (_, result, error) in
                    ...
                    GraphRequest(graphPath: "me/photos", parameters: (param as? [String: Any]) ?? [:], httpMethod: .post).start(completionHandler: { (_, result, error) in
    ...
    func FJPostVideoOnFacebook(_ videoURL: String?, txtMessage: String?, txtDescription: String?, img: UIImage?, completionHandler:@escaping FJFacebookCompletionHandler) {
        let sendRequestPostData = {() -> Void in
            if AccessToken.current?.hasGranted(permission: "publish_actions") ?? false {
                ...
                GraphRequest(graphPath: "me/videos", parameters: (param as? [String: Any]) ?? [:], httpMethod: .post).start(completionHandler: { (_, result, error) in
    ...
    func loginWithReadPermission (completionHandler : @escaping LoginManagerLoginResultBlock) {
        let loginManager: LoginManager = LoginManager()
        loginManager.logIn(permissions: ["public_profile", "email"], from: APP_DELEGATE.window?.rootViewController, handler: { (result, error) in
    ...
    func loginWithPublishPermission (completionHandler : @escaping LoginManagerLoginResultBlock) {
        let loginManager: LoginManager = LoginManager()
        loginManager.logIn(permissions: ["publish_actions"], from: APP_DELEGATE.window?.rootViewController) { (result, error) in
            completionHandler(result, error)
        }
    }
    ...
    func graphAPIWithGraphPath(graphPath: String, params: [String: String]?, completionHandler : @escaping GraphRequestBlock) {
        let token = AccessToken.current?.tokenString
        GraphRequest(graphPath: graphPath, parameters: params ?? [:], tokenString: token ?? "", version: nil, httpMethod: .post).start(completionHandler: { (connection, result, error) in
